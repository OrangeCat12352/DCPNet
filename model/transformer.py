...
class Attention(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        query_layer = self.transpose_for_scores(mixed_query_layer)  # （1，4，121，128）
        key_layer = self.transpose_for_scores(mixed_key_layer)
        value_layer = self.transpose_for_scores(mixed_value_layer)

        attention_scores = torch.matmul(query_layer, key_layer.transpose(-1, -2))  # （1，4，121，121）
        attention_scores = attention_scores / math.sqrt(self.attention_head_size)  # （1，4，121，121）

        attention_probs = self.softmax(attention_scores)
        attention_probs = self.attn_dropout(attention_probs)

        context_layer = torch.matmul(attention_probs, value_layer)  # （1，4，121，128）
        context_layer = context_layer.permute(0, 2, 1, 3).contiguous()
        new_context_layer_shape = context_layer.size()[:-2] + (self.all_head_size,)  # （1，121，512）
        context_layer = context_layer.view(*new_context_layer_shape)
        attention_output = self.out(context_layer)  # （1，121，512）
...
if __name__ == '__main__':
    input1 = torch.rand(8, 512, 8, 8)
    input2 = torch.rand(8, 512, 24, 24)
    model = Transformer(512, 512, 8, 24)
